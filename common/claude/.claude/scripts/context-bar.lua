...
if cwd ~= "" then
    ...
    if branch ~= "" then
        ...
        local porcelain = run(string.format(
            "git -C %q --no-optional-locks status --porcelain 2>/dev/null", cwd))
        local file_count = 0
        local first_file = ""
        ...
        local sync_status
        local upstream = run(string.format(
            "git -C %q rev-parse --abbrev-ref @{upstream} 2>/dev/null", cwd))
        ...
        if upstream ~= "" then
            ...
            local counts = run(string.format(
                "git -C %q rev-list --left-right --count HEAD...@{upstream} 2>/dev/null", cwd))
            ...
            elseif ahead > 0 and behind == 0 then
                sync_status = string.format("%d ahead", ahead)
            elseif ahead == 0 and behind > 0 then
                sync_status = string.format("%d behind", behind)
            else
                sync_status = string.format("%d ahead, %d behind", ahead, behind)
            end
        else
        ...
        if file_count == 0 then
            git_status = string.format("(0 files uncommitted, %s)", sync_status)
        elseif file_count == 1 then
            git_status = string.format("(%s uncommitted, %s)", first_file, sync_status)
        else
            git_status = string.format("(%d files uncommitted, %s)", file_count, sync_status)
        end
    end
end
...
if #transcript_lines > 0 then
    ...
    for i = #transcript_lines, 1, -1 do
        ...
        if entry.message and entry.message.usage
            and not entry.isSidechain
            and not entry.isApiErrorMessage then
...
local ctx = string.format("%s %s%s%d%% of %dk tokens",
    draw_bar(pct), c.gray, pct_prefix, pct, max_k)

-- Build main output line
local output = c.accent .. model .. c.gray .. " |  " .. dir
if branch ~= "" then
...
if #transcript_lines > 0 then
    ...
    local plain = model .. " |  " .. dir
    if branch ~= "" then
        plain = plain .. " |  " .. branch .. " " .. git_status
    end
    plain = plain .. " | xxxxxxxxxx " .. pct .. "% of " .. max_k .. "k"
    local max_len = #plain
    ...
    for i = #transcript_lines, 1, -1 do
        ...
        if entry.type == "user" and entry.message and entry.message.content then
            ...
            if text ~= ""
                ...
                and not text:find("<local%-command%-caveat>", 1, false) then
                last_user_msg = text
                break
            end
        end
